...
func (w *world) Entities(components ...Component) []Entity {

	if len(components) == 0 {
		panic("World.Entities() requires at least on component")
	}
	var types = componentTypes[reflect.TypeOf(components[0])]
	for i := 1; i < len(components); i++ {
		cid := componentTypes[reflect.TypeOf(components[i])]
		types |= cid
	}

	return w.entitiesByTypes(types)
}

func (w *world) entitiesByTypes(componentTypes uint32) []Entity {
	var entities []Entity
